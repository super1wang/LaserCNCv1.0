...
// 中文翻译：输入信号
ProcessNodeDescriptor InputSignalWaitStep::descriptor() const{ ProcessNodeDescriptor d; d.type=ProcessNodeType::InputSignalWait; d.displayName=QObject::tr("input signal"); d.category=QObject::tr("IO"); d.executorKey=QStringLiteral("inputSignalWait"); d.defaultParameters.insert(QString::fromLatin1(kSignalType), QStringLiteral("digital")); d.defaultParameters.insert(QString::fromLatin1(kIoName), QStringLiteral("aStart")); d.defaultParameters.insert(QString::fromLatin1(kTarget), true); d.defaultParameters.insert(QString::fromLatin1(kTimeout), 5000); d.defaultParameters.insert(QString::fromLatin1(kPoll), 100); return d; }
// 中文翻译：等待 %1=%2 timeout=%3ms
QString InputSignalWaitStep::summary(const ProcessNode& node) const{ return QObject::tr("Wait %1=%2 timeout=%3ms").arg(node.parameters.value(kIoName,"aStart").toString(), node.parameters.value(kTarget,true).toString(), node.parameters.value(kTimeout,5000).toString()); }
// 中文翻译：输入类型；IO 名；目标为 true；超时；刷新间隔
QWidget* InputSignalWaitStep::createParameterEditor(const ProcessNode& node, QWidget* parent) const{ auto* page=new QWidget(parent); auto* form=new QFormLayout(page); auto* type=new QComboBox(page); type->setObjectName(kTypeCombo); type->addItems({"digital","analog"}); type->setCurrentText(node.parameters.value(kSignalType,"digital").toString()); form->addRow(QObject::tr("input type"), type); auto* io=new QComboBox(page); io->setObjectName(kIoCombo); refill(io,type->currentText()); const QString current=node.parameters.value(kIoName).toString(); for(int i=0;i<io->count();++i){ if(ioKeyFromDisplay(io->itemText(i))==current){ io->setCurrentIndex(i); break; }} QObject::connect(type,&QComboBox::currentTextChanged,io,[io](const QString& t){ refill(io,t); }); form->addRow(QObject::tr("IO name"), io); auto* target=new QCheckBox(QObject::tr("target is true"),page); target->setObjectName(kTargetCheck); target->setChecked(node.parameters.value(kTarget,true).toBool()); form->addRow(QString(),target); auto* timeout=new QSpinBox(page); timeout->setObjectName(kTimeoutSpin); timeout->setRange(0,24*60*60*1000); timeout->setSuffix(" ms"); timeout->setValue(node.parameters.value(kTimeout,5000).toInt()); form->addRow(QObject::tr("timeout"),timeout); auto* poll=new QSpinBox(page); poll->setObjectName(kPollSpin); poll->setRange(10,60000); poll->setSuffix(" ms"); poll->setValue(node.parameters.value(kPoll,100).toInt()); form->addRow(QObject::tr("refresh interval"),poll); return page; }
...
// 中文翻译：IO 服务不可用
bool InputSignalWaitStep::execute(const ProcessNodeExecutionRequest& request, ProcessStepContext& context, QString* errorMessage){ if(!context.io){ if(errorMessage)*errorMessage=QObject::tr("IO service is not available"); return false;} return context.io->waitInput(request.parameters.value(kSignalType,"digital").toString(), request.parameters.value(kIoName,"aStart").toString(), request.parameters.value(kTarget,true), request.parameters.value(kTimeout,5000).toInt(), request.parameters.value(kPoll,100).toInt(), errorMessage); }
