...
void addAxisRow(QTableWidget* table, const QVariantMap& row = {}) {
    ...
    target->setRange(-1000000, 1000000);
    target->setDecimals(3);
    target->setValue(row.value("target", 0.0).toDouble());
    table->setCellWidget(r, 2, target);
    auto* velocity = new QDoubleSpinBox(table);
    velocity->setRange(0, 1000000);
    velocity->setDecimals(3);
    velocity->setValue(row.value("velocity", 5.0).toDouble());
...
QVariantList rowsFromTable(QTableWidget* table) {
    ...
    for (int r = 0; r < table->rowCount(); ++r) {
        ...
        if (auto* mode = qobject_cast<QComboBox*>(table->cellWidget(r, 1)))
            row.insert(QStringLiteral("mode"), mode->currentData().toString());
        if (auto* target = qobject_cast<QDoubleSpinBox*>(table->cellWidget(r, 2)))
            row.insert(QStringLiteral("target"), target->value());
        if (auto* velocity = qobject_cast<QDoubleSpinBox*>(table->cellWidget(r, 3)))
            row.insert(QStringLiteral("velocity"), velocity->value());
        if (!row.value("axis").toString().isEmpty())
            rows.append(row);
    }
    return rows;
}
...
QString MultiAxisMoveStep::summary(const ProcessNode& node) const {
    // 中文翻译：%1，多轴 %2 项
    return QObject::tr("%1, multi-axis %2 items")
        .arg(node.parameters.value(kMode, "sequential").toString(),
             QString::number(node.parameters.value(kAxes, QVariantList{}).toList().size()));
}
...
QWidget* MultiAxisMoveStep::createParameterEditor(const ProcessNode& node, QWidget* parent) const {
    ...
    mode->setObjectName(kModeBox);
    mode->addItem(QObject::tr("sequential execution"), "sequential");
    mode->addItem(QObject::tr("Synchronous execution"), "sync");
    const int mi = mode->findData(node.parameters.value(kMode, "sequential").toString());
    mode->setCurrentIndex(mi < 0 ? 0 : mi);
    // 中文翻译：多轴模式
    ...
    timeout->setValue(node.parameters.value(kTimeout, 30000).toInt());
    // 中文翻译：超时
    form->addRow(QObject::tr("timeout"), timeout);
    layout->addLayout(form);
    // 中文翻译：轴；模式；目标位置；速度；操作
    auto* table = new QTableWidget(page);
    table->setObjectName(kTable);
    table->setColumnCount(5);
    table->setHorizontalHeaderLabels({QObject::tr("axis"), QObject::tr("mode"),
                                      QObject::tr("Target location"), QObject::tr("speed"),
                                      QObject::tr("Operation")});
    table->horizontalHeader()->setStretchLastSection(true);
    layout->addWidget(table);
    for (const QVariant& v : node.parameters.value(kAxes, QVariantList{}).toList())
...
bool MultiAxisMoveStep::applyParameterEditor(QWidget* editor, ProcessNode& node, QString*) const {
    node.parameters.insert(kMode,
                           editor->findChild<QComboBox*>(kModeBox)->currentData().toString());
...
bool MultiAxisMoveStep::execute(const ProcessNodeExecutionRequest& request,
    ...
    if (rows.isEmpty()) {
        if (errorMessage)
            *errorMessage = QObject::tr("Multi-axis motion axis table is empty");
        return false;
    }
    return context.motion->moveAxes(rows, request.parameters.value(kMode, "sequential").toString(),
                                    request.parameters.value(kTimeout, 30000).toInt(),
                                    errorMessage);
}

int MultiAxisMoveStep::completionDelayMs(const ProcessNodeExecutionRequest&) const {
    return 200;
}
