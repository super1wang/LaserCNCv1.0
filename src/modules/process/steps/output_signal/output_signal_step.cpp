...
// 中文翻译：输出类型；IO 名；值
QWidget* OutputSignalStep::createParameterEditor(const ProcessNode& node, QWidget* parent) const{ auto* page=new QWidget(parent); auto* form=new QFormLayout(page); auto* type=new QComboBox(page); type->setObjectName(kTypeCombo); type->addItems({"digital","analog"}); type->setCurrentText(node.parameters.value(kSignalType,"digital").toString()); form->addRow(QObject::tr("Output type"), type); auto* io=new QComboBox(page); io->setObjectName(kIoCombo); refill(io,type->currentText()); const QString current=node.parameters.value(kIoName).toString(); for(int i=0;i<io->count();++i){ if(ioKeyFromDisplay(io->itemText(i))==current){ io->setCurrentIndex(i); break; }} QObject::connect(type,&QComboBox::currentTextChanged,io,[io](const QString& t){ refill(io,t); }); form->addRow(QObject::tr("IO name"), io); auto* value=new QLineEdit(node.parameters.value(kValue,true).toString(),page); value->setObjectName(kValueEdit); form->addRow(QObject::tr("value"), value); return page; }
...
// 中文翻译：IO 服务不可用
bool OutputSignalStep::execute(const ProcessNodeExecutionRequest& request, ProcessStepContext& context, QString* errorMessage){ if(!context.io){ if(errorMessage)*errorMessage=QObject::tr("IO service is not available"); return false;} return context.io->setOutput(request.parameters.value(kSignalType,"digital").toString(), request.parameters.value(kIoName,"aLaser").toString(), request.parameters.value(kValue,true), errorMessage); }
