...
double PharosLaserDevice::GetAveragePower()
{
	...
	auto LogPowerFailure = [this, &qstrUrl](const QString& qstrReason) {
		...
		if (bWasOk)
		{
			...
			LCNC_ERR(lcnc::LogCode::Generic, "{}", "Pharos GetAveragePower link down, url: " + qstrUrl.toStdString()
				+ ", reason: " + qstrReason.toStdString());
		}
		else if (iNowMs - m_iLastPowerErrorLogMs >= kPowerErrorLogIntervalMs)
		{
			m_iLastPowerErrorLogMs = iNowMs;
			LCNC_WARN(lcnc::LogCode::Generic, "{}", "Pharos GetAveragePower still failing, url: " + qstrUrl.toStdString()
				+ ", reason: " + qstrReason.toStdString());
		}
	};
	...
	auto MarkPowerOk = [this, &qstrUrl]() {
		if (!m_bPowerLinkOk)
		{
			m_bPowerLinkOk = true;
			m_iLastPowerErrorLogMs = 0;
			LCNC_INFO(lcnc::LogCode::Generic, "{}", "Pharos GetAveragePower link recovered, url: " + qstrUrl.toStdString());
		}
	};
	...
	LaserResponseMode eResponseMode, bool bEnableHttpLog)
{
	...
	if (!bOk)
	{
		LCNC_ERR(lcnc::LogCode::Generic, "{}", "Pharos HTTP PUT failed, suffix: " + suffix.toStdString()
			+ ", value: " + strBody
