...
bool ULTRONLaserDevice::SetEnergy(double dEnergy)
{
	...
	if ((dSetEnergy >= 0) && (dSetEnergy <= 100))
	{
		...
		bool bResult = OnceData(data);
		if (!bResult)
			SHOW_OPER_ERROR(ErrorCode::ERROR_LASER_SETTINGFAILED, QObject::tr("Set laser energy %1 failed.").arg(dEnergy).toUtf8().data());
		else
	...
	{
		SHOW_OPER_ERROR(ErrorCode::ERROR_LASER_SETTINGFAILED, QObject::tr("The laser energy %1 does not meet the requirement of 0 to 100.").arg(dEnergy).toUtf8().data());
		return false;
	}
}
...
bool ULTRONLaserDevice::SetFrequency(double dFrequency)
{
	...
	bool bResult = OnceData(data);
	if (!bResult)
		SHOW_OPER_ERROR(ErrorCode::ERROR_LASER_SETTINGFAILED, QObject::tr("Set laser frequency %1 failed.").arg(dFrequency).toUtf8().data());
	else
...
bool ULTRONLaserDevice::SetPulseWidth(double dPulsePickerDivider)
{
	...
	bool bResult = OnceData(data);
	if (!bResult)
		SHOW_OPER_ERROR(ErrorCode::ERROR_LASER_SETTINGFAILED, QObject::tr("Set laser PulsePickerDivider %1 failed.").arg(dPulsePickerDivider).toUtf8().data());
	else
