...
//		m_hHandle = acsc_OpenCommDirect();
	...
	GetModuleFileName(NULL, Configurefile_path, MAX_VALUE);
	...
	//cout<<"filePath_exe:"<<filePath_exe<<endl;

	string debug_filepath(filePath_exe);
	int index = debug_filepath.find_last_of(L'\\');
	string sub = debug_filepath.substr(0, index);
	string addfilepath ="\\Simulator.prg";
	sub +=addfilepath;
