...
bool SimulateCMHPMotionControl::Disconnect()
{
    ...
    const bool stopped = acsc_StopBuffer(m_hHandle, ACSC_NONE, nullptr) != 0;
    const bool closed = acsc_CloseComm(m_hHandle) != 0;
    const bool simulatorClosed = acsc_CloseSimulator() != 0;
    m_hHandle = ACSC_INVALID;
    m_bConnectFlag = false;
    return stopped && closed && simulatorClosed;
}
