	...
	int responseWindowMs, bool bEnableLog)
{
	...
	if (responsePolicy == ResponsePolicy::IgnoreResponseImmediately)
	{
		...
		if (!invoked)
		{
			...
			if (bEnableLog)
			{
				LOG_SYS_ERROR("HTTP queue failed, method: " + method.toStdString()
					+ ", url: " + url.toStdString());
			}
			return false;
		}
	...
	if (!invoked)
	{
		...
		m_eLastCompletion = RequestCompletion::Failed;
		if (bEnableLog)
			LOG_SYS_ERROR("HTTP invoke failed, method: " + method.toStdString() + ", url: " + url.toStdString());
		return false;
	}
	...
	int responseWindowMs, bool bEnableLog)
{
	...
	if (!qurl.isValid() || (qstrScheme != "http" && qstrScheme != "https") || qurl.host().isEmpty())
	{
		...
		if (bEnableLog)
			LOG_SYS_ERROR("HTTP invalid url: " + url.toStdString());
		return false;
	}
	...
	if (bEnableLog)
		LOG_SYS_INFO("HTTP " + method.toStdString() + " request: " + qurl.toString().toStdString());
	...
	if (!reply)
	{
		...
		if (bEnableLog)
			LOG_SYS_ERROR("HTTP unsupported method: " + method.toStdString());
		return false;
	}
	...
	if (responsePolicy == ResponsePolicy::IgnoreResponseImmediately)
	{
		...
		if (bEnableLog)
		{
			LOG_SYS_INFO("HTTP " + method.toStdString() + " dispatched without waiting, url: "
	...
	if (responsePolicy == ResponsePolicy::AcceptNoResponseAfterTimeout && bTimedOut)
	{
		...
		if (bEnableLog)
		{
			LOG_SYS_INFO("HTTP " + method.toStdString() + " timed out waiting for response window, url: "
				+ qurl.toString().toStdString() + ", completion: "
				+ GetLastCompletionText().toStdString());
		}
		return true;
	...
	if (!bSuccess)
	{
		...
		if (bEnableLog)
		{
			LOG_SYS_ERROR("HTTP " + method.toStdString() + " failed, url: "
	...
	else
	{
		...
		if (bEnableLog)
		{
			LOG_SYS_INFO("HTTP " + method.toStdString() + " succeeded, url: "
				+ qurl.toString().toStdString() + ", status: " + std::to_string(iStatusCode)
	...
	const QUrl& qurl, RequestCompletion completion, bool bEnableLog)
{
	...
	auto backgroundHandler = [reply, method, qurl, qstrCompletion, bEnableLog]() {
		...
		if (eError == QNetworkReply::NoError && iStatusCode >= 200 && iStatusCode < 300)
		{
			if (bEnableLog)
			{
				LOG_SYS_INFO("HTTP " + method.toStdString() + " background completion succeeded, url: "
					+ qurl.toString().toStdString() + ", status: " + std::to_string(iStatusCode)
		...
		else
		{
			...
			if (bEnableLog)
			{
				LOG_SYS_ERROR("HTTP " + method.toStdString() + " background completion failed, url: "
					+ qurl.toString().toStdString() + ", status: " + std::to_string(iStatusCode)
					+ ", completion: " + qstrCompletion.toStdString()
					+ ", error: " + qstrError.toStdString());
			}
