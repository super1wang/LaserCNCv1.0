...
bool SerialPort::WriteData(const QByteArray& data, int iTimeOut)
{
    ...
    if (port->isOpen() && port->isWritable()) {
        ...
        if (bytesWritten != data.size()) {
            LOG_SYS_ERROR("bytesWritten != data.size()");
            return false;
        }

        if (QCoreApplication::instance()) {
            if (!port->waitForBytesWritten(iTimeOut)) {
                LOG_SYS_ERROR("waitForBytesWritten timeout");
                return false;
            }
        } else {
            int bytesPerSecond = m_dwBaudRate / 10;
...
bool SerialPort::OnceData(const QByteArray& send, QByteArray& recv, int iTimeOut)
{
    ...
    if (QThread::currentThread() != port->thread()) {
        ...
        QMetaObject::invokeMethod(port, [this, &result]() {
                ...
                if (!IsConnected()) {
                    ...
                    LOG_SYS_ERROR("ONCE Q not connected");
                    return;
                }
                ...
                if (bytesWritten != result.sendData.size()) {
                    ...
                    result.success = false;
                    LOG_SYS_ERROR("ONCE Q write failed");
                    return;
                }
                ...
                if (QCoreApplication::instance()) {
                    if (!port->waitForBytesWritten(result.timeout)) {
                        ...
                        QThread::msleep(20);
                        result.success = false;
                        LOG_SYS_ERROR("ONCE Q waitForBytesWritten timeout");
                        return;
                    }
                    // 添加短暂延迟，让设备有时间准备响应
                    QThread::msleep(20);
    ...
    QMutexLocker locker(&m_mutex);
    if (!IsConnected()) {
        LOG_SYS_ERROR("ONCE B not connected");
        return false;
    }
    ...
    if (bytesWritten != send.size()) {
        ...
        LOG_SYS_ERROR("ONCE B write failed");
        return false;
    }
    ...
    if (QCoreApplication::instance()) {
        if (!port->waitForBytesWritten(iTimeOut)) {
            ...
            // 添加50ms延迟帮助设备恢复
            QThread::msleep(50);
            LOG_SYS_ERROR("ONCE B waitForBytesWritten timeout");
            return false;
        }
        // 添加短暂延迟，让设备有时间准备响应
        QThread::msleep(5);
