...
public:
	...
	wstring m_strType;
	double m_dJunctionVelocity;			// 连接点速度
	double m_dJunctionAngle;			// 连接点角度
	double m_dXsegEndVelocity;			// XSEG末速度
	double m_dCutSmoothTime;			// 切割平滑时间
	double m_dCutSmoothK;				// 切割平滑系数
	double m_dAxisSmoothTime;			// 轴平滑时间
	double m_dAxisSmoothK;				// 轴平滑系数
	double m_dLineVelocity;				// 直线切割速度
	double m_dLineAcc;					// 直线切割加速度
	double m_dLineJerk;					// 直线切割加加速度
	double m_dArcVelocity;				// 圆弧切割速度
	double m_dArcAcc;					// 圆弧切割加速度
	double m_dArcJerk;					// 圆弧切割加加速度
	double m_dIdleXVelocity;			// X轴空程速度（Add）
	double m_dIdleAVelocity;			// A轴空程速度
	double m_dIdleA1Velocity;			// A1轴空程速度
	double m_dIdleYVelocity;			// Y轴空程速度
	double m_dIdleZVelocity;			// Z轴空程速度
	double m_dIdleX1Velocity;			// X1轴空程速度
	double m_dIdleY1Velocity;			// Y1轴空程速度
	double m_dIdleXYAccDec;				// XY轴空程加减速度（Add）
	double m_dIdleXYJerk;				// XY轴空程加加速度（Add）
	double m_dBeforeOn;					// 开激光前的等待时间
	double m_dAfterOn;					// 开激光后的等待时间
	double m_dBeforeOff;				// 关激光前的等待时间
	double m_dAfterOff;					// 关激光后的等待时间
	int m_dLaserFrequency;				// 激光的频率
	int m_dLaserPulseWidth;				// 激光的脉宽
	double m_dLaserEnergy;				// 激光的能量
	double m_dLaserAttenuatorPercentage;	// Pharos 衰减百分比
	double m_dLaserPpDivider;			// Pharos 分频参数
	int m_iLaserDelay;					// Pharos 参数下发延时
	double m_dAnalogValue;
	double m_dRadius;					// 光斑补偿半径
	wstring m_strOffsetType;			// 补偿类型
	double m_dOffsetDiameter;			// 补偿光斑直径
	double m_dOffsetDistance;			// 补偿间距
	double m_dOffsetIgnoreLength;		// 直线圆弧补偿时忽略长度
	double m_dIdleZHeight;
	double m_dCuttingHeight;
	double m_dCuttingHeightCompensate;  // 切割高度补偿
	string m_strDirectionX;
	string m_strDirectionY;

	bool m_bStopBlow;
	double m_dWaitFirst;
	double m_dWaitSecond;
	//Equal Power
	int m_iPDMode;
	double m_dPDScaleFactor;
	double m_dPDWidth;
	double m_dPDPosOffset;
	double m_dPDLowVelMax;
	double m_dPDPosLowVelMax;
	double m_dPDLowVelMax_1;
	double m_dPDPosLowVelMax_1;
	double m_dBlowDelay;				// 吹气打开后等待时间
	double m_dExtend;					// 延长线首端长度
	double m_dExtend_End;				// 延长线末段长度
	bool   m_bTroughFlag;				// 是否盲刻
	int	   m_iTroughBuffer;				// 盲刻执行buffer
	double m_dR0;
	double m_dZ0;
	double m_dFocusOffset;				// 切割时切割嘴距离钢片的距离
	bool   m_bPunch;					// 是否打点
	bool   m_bBlow2;					// 第二路气体

	int m_iPowerSetpoint;
	int m_iRepetitionRate;
	int m_iPulsePickerDivider;
	double m_dAnalogLaserValue;			// 激光器能量参数由模拟量模块儿下发

	bool m_bCuttingHead;
	bool m_bCrossBridge;
	double m_dServoCuttingHeight;

	bool m_bAxisZLinkage;
	double m_dLinkedDelay;
	string m_sLinkedDirection;
	int m_iLinkedMode;
	double m_dLinkageParameterA;
	double m_dLinkageParameterB;
	string m_sLinkedFormula;

	double m_dTroughDelay;				// 盲刻延时
	...
	double m_dXPosition;
	bool m_bX1IsMove;
	double m_dX1Position;
	bool m_bAIsMove;
	double m_dAPosition;
	bool m_bA1IsMove;
	double m_dA1Position;
	bool m_bYIsMove;
	double m_dYPosition;
	bool m_bY1IsMove;
	double m_dY1Position;

	bool m_bFlightCutting;						//飞切
	double m_dFlightCutting_MotorDelay;			//飞切延时
	bool m_bEnergySwitch;						// 工具能量切换标志
};