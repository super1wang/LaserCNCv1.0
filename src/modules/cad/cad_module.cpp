...
bool selectionContainsSketchElement(const lcnc::cad::selection::CadSelectionContext& context,
                                    int sketchId, int elementId) {
...
TopoDS_Shape buildPrimitiveShape(int primitiveIndex, const CadModule::PrimitiveParameters& params,
                                 QString* errMsg) {
...
bool modelCenterForLabels(LcncDocument* doc, const QList<TDF_Label>& labels, gp_Pnt* outCenter,
                          QString* errMsg) {
...
lcnc::cad_algo::TransformParams toAlgorithmTransform(const CadModule::TransformParameters& params,
...
void watchTask(QObject* owner, TaskId taskId, std::function<void(bool)> onFinished) {
    ...
    *connection =
        QObject::connect(lcnc::Kernel::current().taskManager(), &TaskManager::taskFinished, owner,
                         [taskId, onFinished = std::move(onFinished),
                          connection](TaskId finishedId, bool success) mutable {
...
DocumentId ensureTargetDocument(CadModule* module, DocumentId targetDocId,
                                const QString& defaultName, bool* createdNew) {
    if (createdNew)
        *createdNew = false;

    if (targetDocId != kInvalidDocumentId && module->domainDocumentById(targetDocId) != nullptr) {
        return targetDocId;
    }
...
class CadProjectExplorerProjectionAdapter final : public lcnc::cad::ICadProjectExplorerProjection {
  public:
    ...
    projectExplorerActiveSketchElements() const override {
        return m_module.projectExplorerActiveSketchElements();
    }
    QList<lcnc::cad::ProjectExplorerSketch>
    projectExplorerFinishedSketches(DocumentId id) const override {
...
bool CadModule::init(lcnc::IKernel& kernel) {
    ...
    kernel.services().registerBorrowedService<CadModule>(*this);
    kernel.services().registerBorrowedService<lcnc::ICadFacade>(*this);
    ...
    m_documentIoService = std::make_unique<lcnc::cad::CadDocumentIoService>(
        *lcnc::Kernel::current().projectManager(), *lcnc::Kernel::current().taskManager(), this);
    kernel.services().registerBorrowedService<lcnc::cad::CadDocumentIoService>(
        *m_documentIoService);
    ...
    lcnc::Kernel::current().projectManager()->setWorkspaceCloseGuard([this](ProjectWorkspaceId
                                                                                workspaceId) {
        auto* project = lcnc::Kernel::current().projectManager();
        auto* workspace = project ? project->workspace(workspaceId) : nullptr;
        const DocumentId documentId = workspace && workspace->workpieceDocument()
                                          ? workspace->workpieceDocument()->id()
                                          : kInvalidDocumentId;
        if (documentId == kInvalidDocumentId || cancelDocumentTasks(documentId, 10000)) {
            return true;
        }
        LCNC_ERR(
            lcnc::LogCode::Generic,
            "CadModule: workspace {} close deferred because document {} tasks are still running",
...
bool CadModule::cancelDocumentTasks(DocumentId docId, int timeoutMs) {
    auto it = m_documentTaskScopes.find(docId);
    if (it == m_documentTaskScopes.end())
        return true;
    auto* taskManager = lcnc::Kernel::current().taskManager();
    if (!taskManager)
        return false;
    if (!it->second.cancelAndWait(*taskManager, timeoutMs))
        return false;
    m_documentTaskScopes.erase(it);
    return true;
}

void CadModule::trackDocumentTask(DocumentId docId, TaskId taskId) {
    if (taskId == kInvalidTaskId)
        return;
    m_taskScope.track(taskId);
    if (docId != kInvalidDocumentId)
        m_documentTaskScopes[docId].track(taskId);
}

void CadModule::releaseDocumentTask(DocumentId docId, TaskId taskId) {
    m_taskScope.release(taskId);
    auto it = m_documentTaskScopes.find(docId);
    if (it == m_documentTaskScopes.end())
        return;
    it->second.release(taskId);
    if (it->second.empty())
        m_documentTaskScopes.erase(it);
}

CadModule::CadModule(QObject* parent)
    : QObject(parent), m_modelingSession(std::make_unique<lcnc::cad::CadModelingSession>()),
    ...
    connect(project, &lcnc::LcncProjectManager::activeWorkspaceChanged, this,
            [this, project](ProjectWorkspaceId) {
                const DocumentId id = project->workpieceDocumentId();
    ...
    connect(project, &lcnc::LcncProjectManager::projectOpened, this,
            [this, project](const QString&) {
                if (auto* gd = activeGuiDocument()) {
                    gd->rebuildDomain(lcnc::ProjectDomain::Workpiece, project->workpieceDocument());
                    if (auto* md = project->machineDocument())
                        gd->updateMachineWorkspaceTransforms(md, project->workpieceDocument());
    ...
    connect(project, &lcnc::LcncProjectManager::domainDataChanged, this,
            [this, project](lcnc::ProjectDomain domain) {
...
DocumentId CadModule::newDocument(const QString& name) {
    return m_documentIoService ? m_documentIoService->createDocument(name) : kInvalidDocumentId;
}

DocumentId CadModule::openDocument(const QString& filePath) {
    LCNC_DEBUG(lcnc::LogCode::Generic, "CadModule::openDocument begin path={}",
               filePath.toStdString());

    QFileInfo fileInfo(filePath);
    if (filePath.isEmpty() || !fileInfo.exists()) {
        LCNC_WARN(lcnc::LogCode::Generic, "CadModule::openDocument missing file path={}",
                  filePath.toStdString());
        // 中文翻译：打开失败；文件不存在: %1
    ...
    if (!isProjectPackage && ext != "stp" && ext != "step" && ext != "igs" && ext != "iges" &&
        ext != "stl" && ext != "brep") {
        ...
        emit operationFailed(tr("Open failed"),
                             tr("File format not supported yet: %1").arg(fileInfo.suffix()));
        return kInvalidDocumentId;
    }
    ...
    stageTimer->start();
    LCNC_INFO(
        lcnc::LogCode::Generic, "stage=workpiece.load event=begin mode=open format='{}' path='{}'",
        isProjectPackage ? "lcnc" : ext.toStdString(), fileInfo.absoluteFilePath().toStdString());
    ...
    if (isProjectPackage) {
        ...
        const TaskId taskId = lcnc::Kernel::current().taskManager()->run(
            ...
            [filePath, pendingWorkspace, error, loadResult](TaskProgress* prog) {
                ...
                if (!lcnc::LcncProjectPackage::load(*pendingWorkspace->workpieceDocument(), nullptr,
                                                    pendingWorkspace->camDocument(), packagePath,
                                                    loadResult.get(), error.get(),
                                                    pendingWorkspace->camData())) {
                    throw std::runtime_error("project open failed");
                }
                // 中文翻译：生成显示网格...
                prog->setStepName(QStringLiteral("Generate display grid..."));
                if (!lcnc::cad::CadDocumentIoService::prepareDisplayMesh(
        ...
        watchTask(
            this, taskId,
            [this, taskId, filePath, pendingWorkspace, loadResult, error, openGeneration,
             stageTimer](bool success) {
                LCNC_INFO(lcnc::LogCode::Generic,
                          "stage=workpiece.load event=end mode=open format=lcnc result={} "
                          "elapsed_ms={} path='{}'",
                          success ? "success" : "failed", stageTimer->elapsed(),
                          filePath.toStdString());
                releaseDocumentTask(pendingWorkspace->workpieceDocument()->id(), taskId);
                auto* project = lcnc::Kernel::current().projectManager();
                ...
                if (!success) {
                    ...
                    emit operationFailed(tr("Open failed"),
                                         // 中文翻译：无法读取项目文件
                                         error->isEmpty() ? tr("Unable to read project file")
                                                          : *error);
                    return;
                }
                ...
                pendingWorkspace->session().setProjectPath(
                    pendingWorkspace->workpieceDocument()->filePath());
                pendingWorkspace->session().setManifest(loadResult->manifest);
                pendingWorkspace->session().workpiece().displayName = projectName;
                pendingWorkspace->session().workpiece().sourceFilePath =
                    loadResult->manifest.sourceFilePath;
                ...
                if (auto* gd = activeGuiDocument()) {
                    gd->rebuildDomain(lcnc::ProjectDomain::Workpiece, project->workpieceDocument());
                    gd->updateMachineWorkspaceTransforms(project->machineDocument(),
                                                         project->workpieceDocument());
                    gd->fitAll();
                }
            });
        return docId;
...
DocumentId CadModule::importStep(const QString& filePath, DocumentId targetDocId) {
    ...
    const DocumentId docId =
        ensureTargetDocument(this, targetDocId, fileInfo.baseName(), &createdNew);
    ...
    if (task.id == kInvalidTaskId) {
        if (createdNew)
            closeDocument(docId);
        // 中文翻译：导入 STEP 失败
        emit operationFailed(tr("Import STEP failed"), task.error && !task.error->isEmpty()
                                                           ? *task.error
                                                           : tr("Import STEP failed"));
        return kInvalidDocumentId;
    }

    const QString displayName = fileInfo.completeBaseName();
    const QString sourceFilePath = fileInfo.absoluteFilePath();
    trackDocumentTask(docId, task.id);
    watchTask(this, task.id,
              [this, task, taskId = task.id, docId, createdNew, displayName, sourceFilePath,
               stageTimer](bool success) {
                  releaseDocumentTask(docId, taskId);
                  LcncDocument* targetDocument = domainDocumentById(docId);
                  if (success && targetDocument) {
                      success = lcnc::cad::CadDocumentIoService::commitImport(
                          targetDocument, task.payload, task.error.get());
                  } else if (success && task.error) {
                      // 中文翻译：目标文档已关闭
                      *task.error = tr("The target document has been closed");
                      success = false;
                  ...
                  LCNC_INFO(lcnc::LogCode::Generic,
                            "stage=workpiece.load event=end mode=import format=step result={} "
                            "elapsed_ms={} path='{}'",
                            success ? "success" : "failed", stageTimer->elapsed(),
...
DocumentId CadModule::importStl(const QString& filePath, DocumentId targetDocId) {
    ...
    if (task.id == kInvalidTaskId) {
        if (createdNew)
            closeDocument(docId);
        // 中文翻译：导入 STL 失败
        emit operationFailed(tr("Import STL failed"), task.error && !task.error->isEmpty()
                                                          ? *task.error
                                                          : tr("Import STL failed"));
        return kInvalidDocumentId;
    }
    ...
    watchTask(this, task.id,
              [this, task, taskId = task.id, docId, createdNew, displayName, sourceFilePath,
               stageTimer](bool success) {
                  ...
                  LCNC_INFO(lcnc::LogCode::Generic,
                            "stage=workpiece.load event=end mode=import format=stl result={} "
                            "elapsed_ms={} path='{}'",
                            success ? "success" : "failed", stageTimer->elapsed(),
                            sourceFilePath.toStdString());
                  if (!success) {
...
bool CadModule::saveDocument(DocumentId id, const QString& path) {
    ...
    QString err;
    const bool ok = m_documentIoService && m_documentIoService->saveDocument(doc, path, &err);
    if (!ok)
        // 中文翻译：保存失败；保存文档失败
        emit operationFailed(tr("Save failed"),
                             err.isEmpty() ? tr("Failed to save document") : err);
    return ok;
}

void CadModule::exportStep(DocumentId id, const QString& filePath) {
    LcncDocument* doc = domainDocumentById(id);
    const auto task = m_documentIoService ? m_documentIoService->exportStepAsync(doc, filePath)
                                          : lcnc::cad::CadDocumentIoService::ExportTask{};
    if (task.id == kInvalidTaskId) {
        // 中文翻译：导出 STEP 失败
        emit operationFailed(tr("Export STEP failed"), task.error && !task.error->isEmpty()
                                                           ? *task.error
                                                           : tr("Export STEP failed"));
        return;
    }

    trackDocumentTask(id, task.id);
    watchTask(this, task.id, [this, task, taskId = task.id, id](bool success) {
        releaseDocumentTask(id, taskId);
        if (!success) {
            // 中文翻译：导出 STEP 失败
...
void CadModule::closeDocument(DocumentId id) {
    ...
    if (!cancelDocumentTasks(id, 10000)) {
        // 中文翻译：仍有 CAD 后台任务在运行，无法关闭文档
        emit operationFailed(
            tr("Close document failed"),
...
void CadModule::syncSelectionFromView(DocumentId docId) {
    ...
    if (auto* state = m_documentRegistry->ensure(docId)) {
        state->selectionContext() = lcnc::cad::selection::CadSelectionResolver::fromShapeEntries(
            docId, entries, docId != kInvalidDocumentId, isSketchEditing(), hasSelectedSketch(),
            m_selectedSketchId);
    }
...
lcnc::cad::selection::CadSelectionContext CadModule::selectionContext(DocumentId docId) const {
    ...
    if (!entries.isEmpty()) {
        context = lcnc::cad::selection::CadSelectionResolver::fromShapeEntries(
            docId, entries, context.hasDocument, context.sketchEditing, context.hasSelectedSketch,
            context.selectedSketchId);
    } else {
...
void CadModule::setSelectionContext(const lcnc::cad::selection::CadSelectionContext& context) {
    ...
    if (context.hasSelectedSketch && context.selectedSketchId > 0) {
        if (m_selectedSketchDocId != context.docId ||
...
bool CadModule::rotateShape(DocumentId docId, const TDF_Label& label, const gp_Ax1& axis,
                            double angleDeg) {
    return rotateShapes(docId, {label}, axis, angleDeg);
}

bool CadModule::rotateShapes(DocumentId docId, const QList<TDF_Label>& labels, const gp_Ax1& axis,
                             double angleDeg) {
    LcncDocument* doc = domainDocumentById(docId);
    if (!doc || labels.isEmpty())
        return false;
...
TDF_Label CadModule::createShape(DocumentId docId, const TopoDS_Shape& shape, const QString& name,
                                 int entityKind) {
    LcncDocument* doc = domainDocumentById(docId);
    if (!doc)
        return TDF_Label();
...
void CadModule::requestPrimitiveTool(int primitiveIndex) {
    LCNC_DEBUG(lcnc::LogCode::Generic, "CadModule::requestPrimitiveTool primitive={}",
               primitiveIndex);
    emit primitiveToolRequested(primitiveIndex);
}

bool CadModule::buildPrimitivePreview(int primitiveIndex, const PrimitiveParameters& params,
                                      TopoDS_Shape* outShape, QString* errMsg) {
    LCNC_DEBUG(lcnc::LogCode::Generic, "CadModule::buildPrimitivePreview begin primitive={}",
               primitiveIndex);
    if (outShape)
        outShape->Nullify();
    ...
    if (outShape)
        *outShape = shape;
    LCNC_DEBUG(lcnc::LogCode::Generic, "CadModule::buildPrimitivePreview end success=true");
    return true;
}

bool CadModule::createPrimitive(int primitiveIndex, const PrimitiveParameters& params) {
    LCNC_DEBUG(lcnc::LogCode::Generic, "CadModule::createPrimitive begin primitive={}",
               primitiveIndex);

    QString buildError;
    const TopoDS_Shape shape = buildPrimitiveShape(primitiveIndex, params, &buildError);
    if (shape.IsNull()) {
        LCNC_WARN(lcnc::LogCode::Generic, "CadModule::createPrimitive build failed: {}",
                  buildError.toStdString());
        // 中文翻译：创建基础体失败
    ...
    if (docId == kInvalidDocumentId || !domainDocumentById(docId)) {
        LCNC_WARN(lcnc::LogCode::Generic, "CadModule::createPrimitive failed to prepare document");
        // 中文翻译：创建基础体失败；无法创建或激活目标文档
        emit operationFailed(tr("Failed to create base body"),
                             tr("Unable to create or activate target document"));
        return false;
    }

    const TDF_Label label = createShape(docId, shape, primitiveName(primitiveIndex),
                                        static_cast<int>(LcncDocument::EntityKind::Workpiece));
    if (label.IsNull()) {
        LCNC_WARN(lcnc::LogCode::Generic, "CadModule::createPrimitive createShape failed docId={}",
                  docId);
        // 中文翻译：创建基础体失败；无法写入目标文档
        emit operationFailed(tr("Failed to create base body"),
                             tr("Unable to write to target document"));
        return false;
    }

    requestWorkpieceView(docId);
    LCNC_DEBUG(lcnc::LogCode::Generic, "CadModule::createPrimitive end success=true docId={}",
               docId);
    return true;
}

bool CadModule::previewTool(const QString& toolId, const QVariantMap& params,
                            TopoDS_Shape* outShape, QString* errMsg) {
...
bool CadModule::executeTool(const QString& toolId, const QVariantMap& params, QString* errMsg) {
    if (!m_commandDispatcher) {
        if (errMsg)
            // 中文翻译：CAD 工具调度器未初始化
            *errMsg = tr("CAD tool scheduler not initialized");
...
bool CadModule::buildTransformPreview(const TransformParameters& params, TopoDS_Shape* outShape,
    ...
    if (params.referenceMode == 0 && !modelCenterForLabels(doc, labels, &referencePoint, errMsg)) {
        return false;
    }
    if (refX)
    ...
    for (const TDF_Label& label : labels) {
        ...
        const TopoDS_Shape transformed = lcnc::cad::invokeCadAlgorithm(
            [&] { return lcnc::cad_algo::transformShape(shape, algoParams); }, &transformError);
...
bool CadModule::beginSketch(int planeIndex) {
    LCNC_DEBUG(lcnc::LogCode::Generic, "CadModule::beginSketch begin planeIndex={}", planeIndex);
    ...
    if (docId == kInvalidDocumentId || !domainDocumentById(docId)) {
        LCNC_WARN(lcnc::LogCode::Generic, "CadModule::beginSketch failed to prepare document");
        // 中文翻译：新建草图失败；无法创建或激活目标文档
        emit operationFailed(tr("New sketch failed"),
                             tr("Unable to create or activate target document"));
        return false;
    }
    ...
    LCNC_DEBUG(lcnc::LogCode::Generic, "CadModule::beginSketch end success=true docId={}", docId);
    return true;
}
...
bool CadModule::finishSketch() {
    ...
    if (!m_modelingSession) {
        ...
        emit operationFailed(tr("Failed to complete sketch"),
                             tr("CAD modeling session not initialized"));
        return false;
    }

    QString errMsg;
    if (!m_modelingSession->finishSketch(&errMsg)) {
        LCNC_WARN(lcnc::LogCode::Generic, "CadModule::finishSketch failed: {}",
                  errMsg.toStdString());
        // 中文翻译：完成草图失败
    ...
    DocumentId docId = workpieceDocumentId();
    if (docId == kInvalidDocumentId) {
        LCNC_WARN(lcnc::LogCode::Generic, "CadModule::finishSketch no project document active");
        // 中文翻译：完成草图失败；项目文档不可用
        emit operationFailed(tr("Failed to complete sketch"),
                             tr("Project documentation is not available"));
        return false;
    }
    ...
    const int sketchId = manager->addSketch(
        sessionPlane, std::vector<lcnc::cad::SketchElement>(elements.begin(), elements.end()),
        profileFace);
    ...
    if (state) {
        ...
        state->selectionContext() =
            lcnc::cad::selection::CadSelectionResolver::fromProjectExplorerNode(
                docId, QStringLiteral("__sketch_finished_%1__").arg(sketchId), QString(), {});
    }
    emit sketchToolChanged(static_cast<int>(lcnc::cad::SketchToolKind::None));
    emit sketchElementsChanged();
    emit finishedSketchesChanged(docId);
    emit sketchSelectionChanged(sketchId);
    LCNC_DEBUG(lcnc::LogCode::Generic, "CadModule::finishSketch end success=true sketchId={}",
               sketchId);
    return true;
}
...
bool CadModule::applyFeature(int featureIndex, double length, double angleDeg) {
    ...
    if (!manager || m_selectedSketchId <= 0) {
        ...
        emit operationFailed(tr("Apply feature failed"),
                             tr("Please complete and select a sketch first"));
        return false;
    }
    ...
    if (!record || record->profileFace.IsNull()) {
        ...
        emit operationFailed(tr("Apply feature failed"),
                             tr("Selected sketch does not contain usable outlines"));
        return false;
    }
    ...
    if (featureShape.IsNull()) {
        LCNC_WARN(lcnc::LogCode::Generic, "CadModule::applyFeature build failed: {}",
    ...
    const TDF_Label label =
        createShape(docId, featureShape, m_modelingSession->defaultFeatureName(featureKind),
                    static_cast<int>(LcncDocument::EntityKind::Workpiece));
    if (label.IsNull()) {
    ...
    emit finishedSketchesChanged(docId);
    LCNC_DEBUG(lcnc::LogCode::Generic, "CadModule::applyFeature end success=true docId={}", docId);
    return true;
}

bool CadModule::buildFeaturePreview(int featureIndex, double length, double angleDeg,
                                    TopoDS_Shape* outShape, QString* errMsg) {
    if (outShape)
        outShape->Nullify();
...
void CadModule::setSelectedSketchId(int sketchId) {
    ...
    if (auto* state = m_documentRegistry->ensure(m_selectedSketchDocId)) {
        ...
        state->selectionContext() =
            sketchId > 0
                ? lcnc::cad::selection::CadSelectionResolver::fromProjectExplorerNode(
                      m_selectedSketchDocId, QStringLiteral("__sketch_finished_%1__").arg(sketchId),
                      QString(), {})
...
void CadModule::setSketchTool(int toolKind) {
    ...
    if (!m_modelingSession->isSketchEditing()) {
        LCNC_DEBUG(lcnc::LogCode::Generic, "CadModule::setSketchTool ignored: sketch not editing");
        return;
    }
...
bool CadModule::moveSketchElementHandle(int elementId, int handleIndex, double deltaX,
                                        double deltaY, QString* errMsg) {
    if (!m_modelingSession) {
        if (errMsg)
            // 中文翻译：CAD 建模会话未初始化
            *errMsg = tr("CAD modeling session not initialized");
    ...
    QString sessionError;
    if (!m_modelingSession->moveSketchElementHandle(elementId, handleIndex, deltaX, deltaY,
                                                    &sessionError)) {
        if (errMsg)
            *errMsg = sessionError;
        return false;
...
QList<CadModule::SketchOverlaySnapshot> CadModule::sketchOverlaySnapshots(DocumentId docId) const {
    ...
    if (m_modelingSession && m_modelingSession->isSketchEditing() &&
        docId == workpieceDocumentId()) {
    ...
    for (const auto& record : manager->sketches()) {
        ...
        for (const auto& element : record.elements) {
            ...
            snap.visible = record.visible;
            snap.selected =
                sketchSelected || selectionContainsSketchElement(context, record.id, element.id);
...
void CadModule::refreshDisplay(DocumentId docId) {
    ...
    LCNC_DEBUG(lcnc::LogCode::Generic, "CadModule::refreshDisplay docId={} gd={}", docId,
               static_cast<void*>(gd));
    if (gd) {
