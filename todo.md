...
- [ ] 继续将 CAD 入口收敛为生命周期与 facade：`CadDocumentIoService` 已接管新建、保存、关闭及 STEP 导出任务；后续整体迁移 STEP/IGES/STL/BREP 导入、异步取消与显示网格准备，保持成功前不替换活动工程；草图/特征流程和选择刷新仍待 `CadModelingController`、`CadSelectionController` 下沉。
