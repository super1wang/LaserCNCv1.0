...
2026-07-20 最新交付范围、复核证据与按优先级合并后的剩余事项见 `DELIVERY.md`；本文保留分阶段审计记录，出现重复的历史勾选项时以交付说明和源码现状为准。
...
- [x] 建立 `DeviceCommandQueue` 专用设备线程，支持 Stop > Workflow > Interactive > Normal > Polling 与轮询合并；连接、回零、设置、工作流步骤、停止和设备轮询已接入。普通切割在每条轮廓启动后以短状态查询释放租约；断开仍由带生命周期保护的 TaskManager 任务执行。
- [ ] 为 ACS、GTN 和仿真器建立真机/模拟器并发连接、轮询、回零、加工、停止调用序列压力测试。
