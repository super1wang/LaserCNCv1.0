...
- [ ] 完成“加载机台模型”空程规划的 GUI 与离线样件验证。本次提交只确认无机台模型下的模拟锥头/轻量切割嘴、工件最高障碍包络、连续五轴求解、虚线显示和 ACS 指令链；机台模型分支尚未完成操作验证，不能作为真机安全证据。
- [ ] 将机台环境校验从“已求解离散点拒绝”升级为连续段扫掠或保守细分，并在碰撞后生成新的抬高/侧移候选路径；当前实现只在空程采样目标位检查真实头部与工件、机架、夹具距离，既不能证明采样点之间安全，也不会主动绕开机台障碍。
- [ ] 完成机台碰撞角色与忽略对：验证 Z/摆头自动分类是否覆盖轴子树中的未直接归轴子件，增加切割头/工作台/夹具/静态机架角色诊断和显式忽略碰撞对；当前只有 `head`、`obstacle`、`ignore` 部件角色，尚无碰撞对配置。
- [ ] 统一机台环境最小间隙与应用选项，移除真实头部校验中的固定 `0.5 mm`；增加 AABB 宽相、失败部件/姿态/最小间隙诊断，避免大型机台逐形体精确距离计算造成生成刀路卡顿。
- [ ] 在无激光、低速条件下分别验证 AC/BC 转台和摆头构型：机台模型显示/隐藏结果一致，真实切割头与轻量刀嘴基准一致，夹具/卡盘随轴链正确变换，碰撞计划硬阻止加工，并记录安装误差和各控制器实际同步插补行为。
