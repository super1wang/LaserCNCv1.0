...
| 优先级 | 问题 | 影响 | 建议 |
| --- | --- | --- | --- |
| P0 | `LcncProjectPackage::save()` 仍把 `manifest.sourceFilePath` 写成 `workpieceDocument.filePath()`。 | `.lcnc` 保存后工件源文件路径可能变成工程包路径，影响重新打开后的 metadata。 | 调整 package save API，让 `LcncProjectManager` 传入 session/manifest metadata，保存时使用 `session().workpiece().sourceFilePath`。 |
...
| P1 | `.lcnc` package backend 依赖 PowerShell `Compress-Archive` / `Expand-Archive`。 | 对运行环境、错误恢复、编码和大文件稳定性不够可控。 | 抽象 `IPackageArchiveBackend`，替换为 QuaZip/miniz/libzip；增加原子保存和失败回滚。 |
...
- 修复 `.lcnc` 保存 metadata：`manifest.sourceFilePath` 使用 session workpiece source path。
- 建立 smoke 流程：启动、加载机台、导入 STEP、反向顺序加载、三域节点显隐、CAM 轮廓、保存/打开。
- 为 `GuiDocument` 增加 display registry 断言或调试日志：domain rebuild 不得清空其它 domain，entry lookup 必须带 document id。
- 对运行中的 exe 锁定、DLL 缺失、PowerShell package 失败增加更清晰的用户提示。
...
- 再打开 STEP/IGES/STL/BREP 工件后，机台和工件同时显示。
- 反向顺序也成立：先打开工件，再加载机台。
- Machine 节点勾选只影响机台 AIS，不影响工件 AIS。
- Workpiece 节点勾选只影响工件 AIS 和挂载代理。
- CAM 轮廓刷新不清空 Workpiece/Machine 显示。
